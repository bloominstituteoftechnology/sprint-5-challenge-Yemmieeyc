async function sprintChallenge5() { // Note the async keyword, in case you wish to use `await` inside sprintChallenge5
  ...
  async function fetchData() {
    try {
      const theLearners = await getLearner();
      const theMentors = await getMentor();
      learnerCardMaker(theLearners, theMentors);
    } catch (error) {
  ...
  function learnerCardMaker(theLearners, mentors) {
    const fullLearners = theLearners.map(learner => replaceMentorIdsWithNames(learner, mentors));
